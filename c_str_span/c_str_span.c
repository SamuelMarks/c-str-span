 ...
 * version in the az_span.h file as well. */
AZ_NODISCARD az_span az_span_create(uint8_t *ptr, size_t size) {
  az_span span;
...
AZ_NODISCARD az_span az_span_slice_to_end(az_span span, size_t start_index) {
...
AZ_NODISCARD AZ_INLINE uint8_t _az_tolower(uint8_t value) {
  ...
  if ((uint8_t)(int8_t)(value - 'A') <= ('Z' - 'A')) {
    value =
        (uint8_t)((uint32_t)(value + _az_ASCII_LOWER_DIF) & (uint8_t)UINT8_MAX);
  }
  return value;
}
...
AZ_NODISCARD bool az_span_is_content_equal_ignoring_case(az_span span1,
                                                         az_span span2) {
  size_t const size = az_span_size(span1);
  ...
  }
  {
    size_t i;
    for (i = 0; i < size; ++i) {
      if (_az_tolower(az_span_ptr(span1)[i]) !=
          _az_tolower(az_span_ptr(span2)[i])) {
        return false;
      }
    }
  }
...
AZ_NODISCARD az_result az_span_atou64(az_span source, uint64_t *out_number) {
...

  {
    ...
    {
      /* If the first character is not a digit or an optional + sign, return
       * error. */
...

  {
    ...
    { /* If the first character is not a digit, - sign, or an optional + sign,
      ...
      { /* if sign < 0, (-1 * sign + 1) / 2 = 1
         * else, (-1 * sign + 1) / 2 = 0
         * This is necessary to correctly account for the fact that the absolute
         * value of INT32_MIN is 1 more than than the absolute value of
         * INT32_MAX. */
        ...
        for (i = starting_index; i < span_size; ++i) {
          ...
          }
          {
            ...
            /* Check whether the next digit will cause an integer overflow.
             * Before actually doing the math below, this is checking whether
             * value * 10 + d > INT32_MAX, or in the case of negative numbers,
             * checking whether value * 10 + d > INT32_MAX + 1. */
...
#pragma GCC diagnostic pop
     ...
     * out_number argument. */
    return ((size_t)chars_consumed == size && n == 1 &&
            _az_isfinite(*out_number))
               ? AZ_OK
...
AZ_NODISCARD size_t az_span_find(az_span source, az_span target) {
   ...
   * The logic:
   * 1. The function will look into each position of `source` if it contains the
   * same value as the first position of `target`.
   * 2. If it does, it could be that the next bytes in `source` are a perfect
   * match of the remaining bytes of `target`.
   * 3. Being so, it loops through the remaining bytes of `target` and see if
   * they match exactly the next bytes of `source`.
   * 4. If the loop gets to the end (all bytes of `target` are evaluated), it
   * means `target` indeed occurs in that position of `source`.
   * 5. If the loop gets interrupted before cruising through the entire
   * `target`, the function must go back to step 1. from  the next position in
   * `source`. The loop in 5. gets interrupted if
   *     - a byte in `target` is different than `source`, in the expected
   * corresponding position;
   *     - the loop has reached the end of `source` (and there are still
   * remaining bytes of `target` to be checked).
   */
...

  {
    ...
    for (i = 0; i < (source_size - target_size + 1); i++) {
      ...
      if (source_ptr[i] == target_ptr[0]) {
         ...
         * The loop must be broken if it reaches the ends of `target` (step 3.)
         * OR `source` (step 5.). */
        ...
        if (j == target_size) {
          /* All bytes in `target` have been checked and matched the
           * corresponding bytes in `source` (from the start point `i`), so this
...
az_span az_span_copy(az_span destination, az_span source) {
  ...
  { /* Even though the contract of this function is that the destination must be
     * larger than source, cap the data move if the source is too large, to
...
az_span az_span_copy_u8(az_span destination, uint8_t byte) {
  ...
  { /* Even though the contract of the function is that the destination must be
     * at least 1 byte large, no-op if it is empty to avoid memory corruption.
     */
    size_t const dest_size = az_span_size(destination);
    if (dest_size < 1) {
      return destination;
...
void az_span_to_str(char *destination, size_t destination_max_size,
                    az_span source) {
...
AZ_NODISCARD az_result az_span_dtoa(az_span destination, double source,
  ...
  _az_PRECONDITION_RANGE(0, fractional_digits,
  ...
  *out_span = destination;
...

  {
    ...
    _az_RETURN_IF_FAILED(
        _az_span_builder_append_uint64(out_span, (uint64_t)integer_part));
...

    {
      ...
      for (d = 0; d < fractional_digits; d++) {
         ...
         * the decimal point. For example, the decimal part could be 0.00, 0.09,
         * 0.00010, etc. */
...

      {
         ...
         * to be true. */
        _az_PRECONDITION(shifted_fractional_integer_part <=
                         _az_MAX_SAFE_INTEGER);

        { /* The double to uint64_t cast should be safe without loss of
             precision. */
          uint64_t fractional_part = (uint64_t)shifted_fractional_integer_part;

          /* If there is no fractional part (at least within the number of
 ...
 * right */
AZ_NODISCARD static az_span _az_span_trim_side(az_span source,
                                               az_span_trim_side side) {
  int8_t delta = 1;
  uint8_t *source_ptr = az_span_ptr(source);
  size_t const source_size = az_span_size(source);
  ...
  { /* loop source, just to make sure staying within the size range */
    ...
    return az_span_slice_to_end(
        source, index); /* worst case index would be source_size */
  }
}
...
AZ_NODISCARD az_span _az_span_trim_whitespace(az_span source) {
  ...
  return _az_span_trim_whitespace_from_end(
      _az_span_trim_whitespace_from_start(source));
}
...
AZ_NODISCARD AZ_INLINE bool _az_span_is_byte_letter(uint8_t c) {
   ...
   * each other. */
  return ((uint32_t)(c - 'A') & ~(uint32_t)_az_ASCII_SPACE_CHARACTER) <=
         'Z' - 'A';
}
      ...
      for (i = 0; i < source_size; i++) {
        ...
        if (!_az_span_url_should_encode(c)) {
          ...
          *dest_ptr = c;
          ...
          dest_ptr[2] =
              _az_number_to_upper_hex(c & (uint32_t)_az_LARGEST_HEX_VALUE);
          dest_ptr += 3;
        }
      }
