 ...
 *
 *        Public SDK functions validate the arguments passed to them in an
 * effort to ensure that calling code is passing valid values. The valid value
 * is called a contract precondition. If an SDK function detects a precondition
 *        failure (invalid argument value), then by default, it calls a function
 * that places the calling thread into an infinite sleep state; other threads
 *        continue to run.
 *
 *        To override the default behavior, implement a function matching the
 *        az_precondition_failed_fn function signature and then, in your
 * application's initialization (before calling any Azure SDK function), call
 ...
 *
 *        Also, if you define the AZ_NO_PRECONDITION_CHECKING symbol when
 * compiling the SDK code (or adding option -DPRECONDITIONS=OFF with cmake), all
 * of the Azure SDK precondition checking will be excluding making the binary
 * code smaller and faster. We recommend doing this before you ship your code.
 */
...
extern C_STR_SPAN_EXPORT az_precondition_failed_fn
az_precondition_failed_get_callback(void);

/* __analysis_assume() tells MSVC's code analysis tool about the assumptions we
 * have, so it doesn't */
/* emit warnings for the statements that we put into _az_PRECONDITION(). */
/* Code analysis starts to fail with this condition some time around
 * version 19.27; but */
/* __analysis_assume() does appear at least as early as version 1920 (The first
 * "Visual Studio */
/* 2019"). So, we do __analysis_assume() for MSVCs starting with 2019. We don't
 * want to go much */
/* earlier without verifying, because if __analysis_assume() is not available on
 * earlier compiler */
