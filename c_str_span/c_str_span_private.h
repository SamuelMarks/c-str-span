...
enum {
  ...
  /* This many digits can roundtrip between double and uint64_t without loss of
     precision */
  /* or causing integer overflow. We can't choose 16, because 9999999999999999
     is larger than */
...
/**
 * @brief A portable implementation of the standard `isfinite()` function, which
 * may not be available on certain embedded systems that use older compilers.
 *
 * @param value The 64-bit floating point value to test.
 * @return `true` if the \p value is finite (that is, it is not infinite or not
 * a number), otherwise return `false`.
 */
AZ_NODISCARD AZ_INLINE bool _az_isfinite(double value) {
  uint64_t binary_value = 0;

  /* Workaround for strict-aliasing rules. */
  /* Get the 8-byte binary representation of the double value, by
  ...
  /* nan - 0x7FF0000000000001 to 0x7FFFFFFFFFFFFFFF and 0xFFF0000000000001 to
   * 0xFFFFFFFFFFFFFFFF */

  /* This is equivalent to checking the following ranges, condensed into a
   * single check: */
...
/**
 * @brief Removes all leading and trailing whitespace characters from the \p
 * span. Function will create a new #az_span pointing to the first
 * non-whitespace (` `, \\n, \\r, \\t) character found in \p span and up to the
 * last non-whitespace character.
 *
 * @remarks If \p span is full of non-whitespace characters, this function will
 * return empty #az_span.
 *
 * Example:
 * \code{.c}
...
/* \endcode
 *
 * @param[in] source #az_span pointing to a memory address that might contain
 * whitespace characters.
 * @return The trimmed #az_span.
 */
extern C_STR_SPAN_EXPORT AZ_NODISCARD az_span
_az_span_trim_whitespace(az_span source);
 ...
 * `, \\n, \\r, \\t) character found in \p span and up to the last character.
 *
 * @remarks If \p span is full of non-whitespace characters, this function will
 * return empty #az_span.
 *
 ...
 */
extern C_STR_SPAN_EXPORT AZ_NODISCARD az_span
_az_span_trim_whitespace_from_start(az_span source);

/**
 * @brief Removes all trailing whitespace characters from the end of \p span.
 * Function will create a new #az_span pointing to the first character in \p
 * span and up to the last non-whitespace (` `, \\n, \\r, \\t) character.
 *
 * @remarks If \p span is full of non-whitespace characters, this function will
 * return empty #az_span.
 *
 ...
 */
extern C_STR_SPAN_EXPORT AZ_NODISCARD az_span
_az_span_trim_whitespace_from_end(az_span source);
